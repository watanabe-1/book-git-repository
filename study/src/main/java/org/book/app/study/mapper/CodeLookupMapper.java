...
public interface CodeLookupMapper {
   ...
   * 1行update
   * プライマルキーをWhere句に指定
   * プライマルキー：@Param("listNameWhere")String listName, @Param("codeWhere")String code
   * @param cod entity(CodeLookup)
   * @param listName LIST_NAME(リストネーム)
   * @param code CODE(コード)
   * @return update行数
   */
  int updateOne(@Param("cod") CodeLookup cod, @Param("listNameWhere")String listName, @Param("codeWhere")String code);
