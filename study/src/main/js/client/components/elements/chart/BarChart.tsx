...
const BarChart: React.FC<BarChartProps> = ({ data, date, topTitle }) => {
  ...
  const maxData: number = useMemo(
    () => Math.max(...(data.datasets[0].data as number[])),
    [data.datasets[0].data]
  );
  const dateStr = useMemo(() => format(date, 'yyyy/MM'), [date]);
        ...
        tooltip: {
          ...
          callbacks: {
            title: function (context): string {
              return `${context[0].label}(${dateStr})`;
            },
  ...
  const plugins: Plugin<'bar', AnyObject>[] = useMemo(
    () => [
      {
        id: 'studyBarChart',
        //棒の頭にラベルをつける
